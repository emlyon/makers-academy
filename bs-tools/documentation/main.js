addEventListener( 'load', e => {
    document.getElementById( 'submit-links' ).addEventListener( 'click', e => {
        ...
        if ( document.querySelector( '#documentation' ).value.indexOf( 'glitch.com/' ) == -1 ) {
            document.getElementById( 'documentation-invalid' ).style.display = 'block';
            return false;
        }
        else if( brightspace ) {
            ...
            let submission = new Request( 'https://script.google.com/macros/s/AKfycbxOZyix_ss0JujTv9j1RiCBzJUQdNMabvfg939w/exec', {
                method: 'POST',
                headers: new Headers( {
